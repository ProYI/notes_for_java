...
public class ResponseDataDemo1 extends HttpServlet {
    ...
    private void test3(HttpServletResponse response)
        ...
        // 注意，下面写错了会提示下载
        // 正确的是：response.setHeader("Content-type", "text/html;charset=UTF-8");
    ...
    // 使用HTML技术中meta标签模拟了一个http响应头，来控制浏览器行为
    private void test2(HttpServletResponse response)
            throws IOException, UnsupportedEncodingException {
        // html <meta>标签可以模拟一个http响应头，所以实际开发中可以不发送相应头消息，通过<meta>标签模拟
...
@WebServlet("/ResponseDownloadDemo")
public class ResponseDownloadDemo extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String path = this.getServletContext().getRealPath("/download/刘亦菲.jpg");
...
@WebServlet("/ResponseImgDemo")
public class ResponseImgDemo extends HttpServlet {
...
<body>
	<form action="">
		...
		认证码：<input type="text" name="checkcode"><img alt="更换验证码" src="/firstweb/ResponseImgDemo" onclick="changeImage(this)"style="cursor: hand"><br /> 
...
// 控制浏览器定时刷新
@WebServlet("/ResponseRefreshDemo")
public class ResponseRefreshDemo extends HttpServlet {
	...
	private void test3(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        // 假设这是一个处理登录的servlet，程序运行到此，用户登录成功
	    
	    // 实际开发中用meta标签模拟消息头
        String message = "<meta http-equiv='refresh' content='3;url=/firstweb/register.html'>恭喜你，登录成功，本浏览器将在3秒后跳转到首页，如果没有跳转，请点击<a href=''>超链接</a>";
    ...
    private void test2(HttpServletResponse response) throws IOException {
	    ...
	    response.getWriter().write("恭喜你，登录成功，本浏览器将在3秒后跳转到首页，如果没有跳转，请点击<a href=''>超链接</a>");
    }
...
@WebServlet("/ResponseCacheDemo")
public class ResponseCacheDemo extends HttpServlet {
...
/*
 * 重定向的特点：
 * 1. 浏览器会向服务器发送两次请求，意味着就有2个request和response
 * 2. 用重定向技术，浏览器地址栏会发生变化
 * 
 * 用户登录和显示购物车时，通常会用到重定向技术
 */
@WebServlet("/ResponseRedirectDemo")
